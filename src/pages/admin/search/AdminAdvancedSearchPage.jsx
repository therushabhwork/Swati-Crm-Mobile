...
import { getCrmOwnerCode, getCrmOwnerDisplay } from '../../../features/users/crmUserDirectory'
...
const buildAccountContacts = (account) => {
  const accountOwner = account.accountOwnerDisplay || account.accountOwnerName || account.accountOwner || '-'
  const accountOwnerCode = account.accountOwnerCode || getCrmOwnerCode(accountOwner) || '-'
  ...
  return sourceContacts
    ...
    .map((contact, index) => ({
      ...
      accountNumber: accountOwnerCode,
                ...
                if (column.isRecordNumber) {
                  return (
                    <td key={column.key}>
                      ...
                      >
                        {renderRecordLinkContent(column, row, value)}
                      </button>
                    </td>
                  )
                }
