...
const formatSummaryCellValue = (columnKey, value) => {
...
const getQuotationSerialNumber = (quotationNumber) => {
  const digits = String(quotationNumber ?? '').match(/\d+/g)?.join('')
  return digits ? digits.slice(-4) : String(quotationNumber ?? '-').replace(/\//g, '')
}

const getQuotationSerialSortValue = (quotationNumber) => {
  const serial = getQuotationSerialNumber(quotationNumber)
  const numberValue = Number(serial)
  ...
  return (
    <div className="qsr-row-action" ref={ref}>
      <button type="button" className="qsr-row-badge" onClick={(e) => { e.stopPropagation(); onToggle(row.num) }}>
        {getQuotationSerialNumber(row.num)}
...
const ResultsTable = ({
  ...
  const sorted = useMemo(() => {
    ...
    if (addOrderBy === 'NO') {
      list.sort((a, b) => {
        const serialA = getQuotationSerialSortValue(a.num)
        const serialB = getQuotationSerialSortValue(b.num)
        if (serialA !== serialB) return serialA - serialB

        const timeA = parseDateString(a.date)
        const timeB = parseDateString(b.date)
        return showLatestQuotations === 'YES' ? timeB - timeA : timeA - timeB
      })
    } else {
      ...
      list.sort((a, b) => {
        ...
        if (key === 'num') {
          valA = getQuotationSerialSortValue(a.num)
          valB = getQuotationSerialSortValue(b.num)
        } else if (key === 'date') {
              ...
              slice.map((row) => (
                <tr key={row.num} className="qsr-row">
                  {selectedColumns.map((col) => {
                    ...
                    return <td key={col.key} className={cellClassName}>{formatSummaryCellValue(col.key, row[col.key])}</td>
                  })}
                </tr>
              ))
            )}
...
const QuotationSummaryReportPage = ({ basePath }) => {
  ...
  /* Sorting state */
  const [sortConfig, setSortConfig] = useState({ key: 'num', direction: 'asc' })
  ...
  const liveRows = useMemo(() => (
    quotations.map((quotation, index) => {
      ...
      return normalizeRow({
        ...
        accountNumber: formatSummaryCellValue('accountNumber', quotation.clientAccountNumber || linkedAccount?.accountNumber || ''),
  ...
  const buildExportRows = (rowsToExport, columnsToExport) => rowsToExport.map((row) => {
    ...
    columnsToExport.forEach((column) => {
      ...
      exportRow[column.label] = formatSummaryCellValue(column.key, row[column.key])
    })
    return exportRow
  })
