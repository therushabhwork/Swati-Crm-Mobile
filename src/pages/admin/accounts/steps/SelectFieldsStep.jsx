...
import { BULK_UPLOAD_FIELD_OPTIONS } from '../../../../features/adminAccounts/bulkUpload/constants'
import { validateMappings } from '../../../../features/adminAccounts/bulkUpload/validators'
...
const SelectFieldsStep = ({ csvHeaders, onMappingsChange, initialMappings, error, setError }) => {
  ...
  useEffect(() => {
    // Auto-map headers that match field names (fuzzy matching)
    const autoMappings = {}
    csvHeaders.forEach((header) => {
      const normalized = header.toLowerCase().replace(/\s+/g, '')
      const match = BULK_UPLOAD_FIELD_OPTIONS.find(
  ...
  const handleMappingChange = (csvHeader, fieldValue) => {
    ...
    // Validate mappings
    const validation = validateMappings(newMappings, csvHeaders)
  ...
  return (
    <div className="bulk-upload-step bulk-upload-step-select-fields">
      <h2 className="bulk-upload-step-title">Map your CSV columns to account fields</h2>
        ...
        {csvHeaders.map((header) => (
          <div key={header} className="bulk-upload-mapping-row">
            ...
            >
              ...
              {BULK_UPLOAD_FIELD_OPTIONS.map((option) => (
      ...
      <div className="bulk-upload-mapping-info">
        <p>Mapped columns: <strong>{Object.keys(mappings).length} / {csvHeaders.length}</strong></p>
      </div>
    </div>
  )
}
